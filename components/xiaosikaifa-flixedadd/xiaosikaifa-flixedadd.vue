<template>
	<view>
		...
		<view @tap="addtap()" :animation="animationadd" class="xiaosikaifa_add"
			:style="{backgroundColor:showBackgroundColor,color:showColor}">+</view>
		<view v-if="xiaosikaifa_add_list_flag" :animation="animationaddlist" class="xiaosikaifa_add_list">


			<view v-for="(listvalue, index) in addlistdata" :key="index" class="xiaosikaifa_add_list_item"
				@tap="addlisttap(index)">
				<text>{{listvalue.title}}</text>
				<image :src="listvalue.src" style="width: 20px;height: 20px;"></image>
			</view>
