<template>
	<view>
		<view class="mx-2"><liu-search-bar :mode="1" @confirm="confirm" @blur="blur" @input="input"
				placeholder="请输入您想搜索的攻略"></liu-search-bar>
		</view>
	...
	export default {
		...
		onLoad() {
			if (this.list.length === 0) {
				this.list = JSON.parse(uni.getStorageSync('historySearchText2'))
			}
		},
		...
		methods: {
			...
			confirm(e) {
				...
				if (this.searchText !== "") {
					...
					// 保存到本地存储
					if (this.list.length !== 0) {
						uni.setStorageSync('historySearchText2', JSON.stringify(this.list))
					}
					// 处于加载中状态
					uni.showLoading({
						title: '加载中',
					...
					// 请求搜索
					uni.navigateTo({
						url: '/pages/search-strategy-list/search-strategy-list?searchText=' + this.searchText,
