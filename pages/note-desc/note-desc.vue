	...
	<view>
		<view class="cu-bar bg-transparent search">
			<view class="cu-avatar round " @click.stop="userDetail(desc.userId)">
				<image :src="randomAvatar(desc.userId)" mode="widthFix" class="round"></image>
			</view>
		...
		<imgsBanner-tag v-if="desc.postImageList" :imgList='swiperList' autoplay duration='500' interval="5000"
		...
		<view style="padding: 40rpx;">
			...
			<view class="flex justify-between">
				...
				<!-- 				<view class="iconfont icon-weizhi" style="line-height: 1.2;font-size: 25rpx;" @click="placePost">
					{{desc.recommendPlace}}
				</view> -->
			</view>
			<!-- 点赞 收藏帖子 收藏地点 -->
			<view class="flex justify-between" style="margin-top: 20rpx;">
				...
				<view class="action" @click="relate" v-if="desc.strategyId">
					<view class="padding-xs">
		...
		<!-- 占位 -->
		<view class="music-button" @click="toggleAudio">
			<image v-if="isPlaying" class="music-icon" src="@/static/pause.png" />
			<image v-else class="music-icon" src="@/static/play.png" />
		</view>
	...
	export default {
		...
		data() {
			return {
				...
				isPlaying: false,
				audio: null,
				audioUrl: 'http://110.40.182.65/music/1.mp3', // 替换成你的音频链接
				TabCur: 0,
				scrollLeft: 0,
		...
		methods: {
			...
			relate() {
				uni.navigateTo({
					url: '/pages/stragety-detail4/stragety-detail4?id=' + this.desc.strategyId
				})
			},
			randomAvatar(userId) {
			...
			placePost() {
				uni.request({
					url: 'http://110.40.182.65:8080/post/find/' + this.placeId + '/post',
					method: 'GET',
					success: res => {
			...
			collectPlace() {
				...
				if (this.placeCollected == true) {
					uni.showModal({
						...
						success: res => {
							...
							if (res.confirm == true) {
								//取消收藏
								uni.request({
									url: 'http://110.40.182.65:8080/address/cancel/' + this.placeId +
										'/collect',
									method: 'PUT',
									data: {},
									success: res => {
					...
					//原本没收藏，收藏
					uni.request({
						url: 'http://110.40.182.65:8080/address/' + this.placeId + '/collect',
						method: 'PUT',
						data: {},
						success: res => {
							console.log(res);
			...
			likePost() {
				...
				if (this.liked == true) {
					uni.showModal({
						...
						success: res => {
							...
							if (res.confirm == true) {
								//取消收藏
								uni.request({
									url: 'http://110.40.182.65:8080/post/cancel/' + this.postId +
										'/like',
									method: 'PUT',
									data: {},
									success: res => {
					...
					//原本没关注，关注
					uni.request({
						url: 'http://110.40.182.65:8080/post/' + this.postId + '/like',
						method: 'PUT',
						data: {},
						success: res => {
							console.log(res);
			...
			collectPost() {
				...
				if (this.collected == true) {
					...
					//原本没关注，关注
					uni.request({
						url: 'http://110.40.182.65:8080/post/' + this.postId + '/collect',
						method: 'PUT',
						data: {},
						success: res => {
							console.log(res);
